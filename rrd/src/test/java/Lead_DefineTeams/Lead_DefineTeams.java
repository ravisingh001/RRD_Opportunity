 ...
 * </p>
 * <p>
 * 5. Select Record type - Lead criteria and clicks on Next button.
 * </p>
 * <p>
 * 6.New Team: Lead Criteria pop up appears .
 * </p>
 * <p>
...
public class Lead_DefineTeams {
	WebDriver driver;
	ExtentReports report;
	ExtentTest logger;
	Properties pro;
	public String Lead_Testdata_sheet_path = System.getProperty("user.dir") + File.separator + "AppData"
			+ File.separator + "Lead_DefineTeams.xlsx";

	@BeforeMethod
	public void setUp() throws FileNotFoundException, InterruptedException {
		report = new ExtentReports("./Report/Lead_DefineTeamsSetUp.html");
		logger = report.startTest("Verify Lead add  Define Team Setup ");
	...
	public void DefineTeamSetup() throws InterruptedException {
		...
		int KeywordrowCount = reader.getRowCount("Lead_Keywords");
		int RRDTeamrowCount = reader.getRowCount("Lead_Team");
		int Criteriacount = reader.getRowCount("Lead_TeamNewCriteria");
		int RRDusercount = reader.getRowCount("Lead_User");
		for (int rowNum = 2; rowNum <= KeywordrowCount; rowNum++) {
			...
			for (int rownum = 2; rownum <= RRDTeamrowCount; rownum++) {
				...
				try {
					...
					if ((reader.getCellData("Lead_Keywords", "Team", rowNum))
							.equalsIgnoreCase(reader.getCellData("Lead_Team", "TeamName", rownum)))

					{

						String SpriorityOrder = reader.getCellData("Lead_Team", "PriorityOrder", rownum);
						String SteamName = reader.getCellData("Lead_Team", "TeamName", rownum);
						String SmaxIntakeLimit = reader.getCellData("Lead_Team", "Max_Intake_limit", rownum);
						 ...
						 */
						Thread.sleep(5000);
						teamobj.clickonLeadsCriteria_RadioButton();
						logger.log(LogStatus.INFO, "Succesfully selected Leads radio button");
						Thread.sleep(5000);
						teamobj.clickOnNext();
			...
			for (int rownumt = 2; rownumt <= Criteriacount; rownumt++) {
				try {
					...
					if ((reader.getCellData("Lead_Keywords", "Team", rowNum))
							.equalsIgnoreCase(reader.getCellData("Lead_TeamNewCriteria", "TeamName", rownumt)))

					{

						String Scriteria = reader.getCellData("Lead_TeamNewCriteria", "RRD_Crieteria", rownumt);
						String Soperator = reader.getCellData("Lead_TeamNewCriteria", "RRD_operator", rownumt);
						String Sfieldval = reader.getCellData("Lead_TeamNewCriteria", "RRD_fieldvalue", rownumt);
						String logic = reader.getCellData("Lead_TeamNewCriteria", "CriteriaLogic", rownumt);
			...
			for (int rowval = 2; rowval <= RRDusercount; rowval++) {
				try {
					...
					if ((reader.getCellData("Lead_Keywords", "Team", rowNum))
							.equalsIgnoreCase(reader.getCellData("Lead_User", "TeamName", rowval))) {

						String RRdUser = reader.getCellData("Lead_User", "SearchPeople", rowval);
						String caseDate = reader.getCellData("Lead_User", "LastCaseAssignedDate", rowval);
						String Leaddate = reader.getCellData("Lead_User", "LastLeadAssignedDate", rowval);
						String Oppdate = reader.getCellData("Lead_User", "LastOppAssignedDate", rowval);
						String Recordlimit = reader.getCellData("Lead_User", "RecordAssignLimit", rowval);
						String AllowedNo = reader.getCellData("Lead_User", "Allowedopencases", rowval);
