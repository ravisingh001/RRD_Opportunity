...
public class Lead_Config22 {
	...
	public void setUp() throws FileNotFoundException, InterruptedException {
		report = new ExtentReports("./Report/Lead_Configuration22.html");
		logger = report.startTest("Verify Lead Configuration22");
	...
	@Test
	public void Test_Config22() throws InterruptedException {

		Xls_Reader reader = new Xls_Reader(Lead_Testdata_sheet_path);

		int rowCount = reader.getRowCount("Lead_Config22");
		for (int rowNum = 2; rowNum <= rowCount; rowNum++) {

			String sUser1 = reader.getCellData("Lead_Config22", "Enable_RRD_Users", rowNum);
			String sOwnerNotification = reader.getCellData("Lead_Config22", "ownerNotification", rowNum);
			String sReleationshipBasedAssignment = reader.getCellData("Lead_Config22", "Rel_Based_Asgmt", rowNum);
			// String RBA_LeadField = reader.getCellData("Lead_Config22", "Lead Fields",
			// rowNum);
			...
			// logger.log(LogStatus.INFO, "Select Relationship based assignment Lead Field
			// value from drop down");

			Xls_Reader Reader = new Xls_Reader(Lead_ConfigCriteria_sheet_path);
			int Criteriacount = Reader.getRowCount("Lead_Config22_CL");
			...
			for (int rownumt = 2; rownumt <= Criteriacount; rownumt++) {
				try {
					String Scriteria = Reader.getCellData("Lead_Config22_CL", "RRD_Crieteria", rownumt);
					String Soperator = Reader.getCellData("Lead_Config22_CL", "RRD_operator", rownumt);
					String Sfieldval = Reader.getCellData("Lead_Config22_CL", "RRD_fieldvalue", rownumt);
					 //String logic = Reader.getCellData("Lead_Config22_CL", "CriteriaLogic",rownumt);
