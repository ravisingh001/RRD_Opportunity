...
public class Lead_Config17 {
	...
	public void setUp() throws FileNotFoundException, InterruptedException {
		report = new ExtentReports("./Report/Lead_Configuration17.html");
		logger = report.startTest("Verify Lead Configuration17");
	...
	@Test
	public void Test_Config17() throws InterruptedException {


		Xls_Reader reader = new Xls_Reader(Lead_Testdata_sheet_path);

		int rowCount = reader.getRowCount("Lead_Config17");
		for (int rowNum = 2; rowNum <= rowCount; rowNum++) {
			String sUser1 = reader.getCellData("Lead_Config17", "Enable_RRD_Users", rowNum);
			String sOwnerNotification = reader.getCellData("Lead_Config17", "ownerNotification", rowNum);
			String sReleationshipBasedAssignment = reader.getCellData("Lead_Config17", "Rel_Based_Asgmt", rowNum);
			String RBA_LeadField = reader.getCellData("Lead_Config17", "Lead Fields", rowNum);
			...
			int Criteriacount = Reader.getRowCount("Lead_Config17_CL");
			...
			for (int rownumt = 2; rownumt <= Criteriacount; rownumt++) {
				try {
				String Scriteria = Reader.getCellData("Lead_Config17_CL", "RRD_Crieteria", rownumt);
				String Soperator = Reader.getCellData("Lead_Config17_CL", "RRD_operator", rownumt);
				String Sfieldval = Reader.getCellData("Lead_Config17_CL", "RRD_fieldvalue", rownumt);
				//String logic = Reader.getCellData("Lead_Config17_CL", "CriteriaLogic", rownumt);
