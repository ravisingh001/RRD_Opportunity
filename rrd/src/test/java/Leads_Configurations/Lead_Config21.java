...
public class Lead_Config21 {
	...
	public void setUp() throws FileNotFoundException, InterruptedException {
		report = new ExtentReports("./Report/Lead_Configuration21.html");
		logger = report.startTest("Verify Lead Configuration21");
	...
	@Test
	public void Test_Config21() throws InterruptedException {

		Xls_Reader reader = new Xls_Reader(Lead_Testdata_sheet_path);

		int rowCount = reader.getRowCount("Lead_Config21");
		for (int rowNum = 2; rowNum <= rowCount; rowNum++) {
			String sUser1 = reader.getCellData("Lead_Config21", "Enable_RRD_Users", rowNum);
			String sOwnerNotification = reader.getCellData("Lead_Config21", "ownerNotification", rowNum);
			String sReleationshipBasedAssignment = reader.getCellData("Lead_Config21", "Rel_Based_Asgmt", rowNum);
			String RBA_LeadField = reader.getCellData("Lead_Config21", "Lead Fields", rowNum);
			...
			int Criteriacount = Reader.getRowCount("Lead_Config21_CL");
			...
			for (int rownumt = 2; rownumt <= Criteriacount; rownumt++) {
				try {
					String Scriteria = Reader.getCellData("Lead_Config21_CL", "RRD_Crieteria", rownumt);
					String Soperator = Reader.getCellData("Lead_Config21_CL", "RRD_operator", rownumt);
					String Sfieldval = Reader.getCellData("Lead_Config21_CL", "RRD_fieldvalue", rownumt);
					 //String logic = Reader.getCellData("Lead_Config21_CL", "CriteriaLogic",rownumt);
