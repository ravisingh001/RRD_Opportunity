...
public class Lead_Config16 {
	...
	public void setUp() throws FileNotFoundException, InterruptedException {
		report = new ExtentReports("./Report/Lead_Configuration16.html");
		logger = report.startTest("Verify Lead Configuration16");
	...
	@Test
	public void Test_Config16() throws InterruptedException {

		Xls_Reader reader = new Xls_Reader(Lead_Testdata_sheet_path);

		int rowCount = reader.getRowCount("Lead_Config16");
		for (int rowNum = 2; rowNum <= rowCount; rowNum++) {
			String sUser1 = reader.getCellData("Lead_Config16", "Enable_RRD_Users", rowNum);
			String sOwnerNotification = reader.getCellData("Lead_Config16", "ownerNotification", rowNum);
			String sReleationshipBasedAssignment = reader.getCellData("Lead_Config16", "Rel_Based_Asgmt", rowNum);
			String RBA_LeadField = reader.getCellData("Lead_Config16", "Lead Fields", rowNum);
			...
			for (int rownumt = 2; rownumt <= Criteriacount; rownumt++) {
				try {
				String Scriteria = Reader.getCellData("Lead_Config16_CL", "RRD_Crieteria", rownumt);
				String Soperator = Reader.getCellData("Lead_Config16_CL", "RRD_operator", rownumt);
				String Sfieldval = Reader.getCellData("Lead_Config16_CL", "RRD_fieldvalue", rownumt);
				//String logic = Reader.getCellData("Lead_Config16_CL", "CriteriaLogic", rownumt);
