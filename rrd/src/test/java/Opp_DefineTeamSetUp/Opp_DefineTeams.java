 ...
 * </p>
 * <p>
 * 5. Select Record type - opportunity criteria and clicks on Next button.
 * </p>
 * <p>
 * 6.New Team: opportunity Criteria pop up appears .
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * 22.Fill the Last assigned Opportunity date
 * </p>
 * <p>
 * 22.Fill the Last assigned Opportunity Time
 * </p>
 * <p>
 * 22.Fill the Record limit
 * </p>
 * <p>
 * 22.Fill the Allowed no. of open opportunity
 * </p>
 * <p>
...
public class Opp_DefineTeams {
	WebDriver driver;
	ExtentReports report;
	ExtentTest logger;
	Properties pro;
	public String Opp_Testdata_sheet_path = System.getProperty("user.dir") + File.separator + "AppData" + File.separator
			+ "Opp_DefineTeamSetUp.xlsx";

	@BeforeMethod
	public void setUp() throws FileNotFoundException, InterruptedException {
		report = new ExtentReports("./Report/Opp_DefineTeamSetup.html");
		logger = report.startTest("Verify OPP add  Define Team Setup ");
	...
	public void DefineTeamSetup() throws InterruptedException {
		...
		int KeywordrowCount = reader.getRowCount("Opp_Keywords");

		int RRDTeamrowCount = reader.getRowCount("Opp_RRDTeam");

		int Criteriacount = reader.getRowCount("Opp_TeamNewCriteria");

		int RRDusercount = reader.getRowCount("Opp_RRDUser");
		...
		for (int rowNum = 2; rowNum <= KeywordrowCount; rowNum++) {
			...
			for (int rownum = 2; rownum <= RRDTeamrowCount; rownum++) {
				...
				try {
					...
					if ((reader.getCellData("Opp_Keywords", "Team", rowNum))
							.equalsIgnoreCase(reader.getCellData("Opp_RRDTeam", "TeamName", rownum)))

					{

						String SpriorityOrder = reader.getCellData("Opp_RRDTeam", "PriorityOrder", rownum);
						String SteamName = reader.getCellData("Opp_RRDTeam", "TeamName", rownum);
						String SmaxIntakeLimit = reader.getCellData("Opp_RRDTeam", "Max_Intake_limit", rownum);
						 ...
						 */
						Thread.sleep(5000);
						teamobj.clickonOpportunityCriteria_RadioButton();
						logger.log(LogStatus.INFO, "Succesfully clicked on  RRD New team button");
						Thread.sleep(5000);
						teamobj.clickOnNext();
			...
			for (int rownumt = 2; rownumt <= Criteriacount; rownumt++) {
				try {
					...
					if ((reader.getCellData("Opp_Keywords", "Team", rowNum))
							.equalsIgnoreCase(reader.getCellData("Opp_TeamNewCriteria", "TeamName", rownumt)))

					{

						String Scriteria = reader.getCellData("Opp_TeamNewCriteria", "RRD_Crieteria", rownumt);
						String Soperator = reader.getCellData("Opp_TeamNewCriteria", "RRD_operator", rownumt);
						String Sfieldval = reader.getCellData("Opp_TeamNewCriteria", "RRD_fieldvalue", rownumt);
						String logic = reader.getCellData("Opp_TeamNewCriteria", "CriteriaLogic", rownumt);
			...
			for (int rowval = 2; rowval <= RRDusercount; rowval++) {
				try {
					...
					if ((reader.getCellData("Opp_Keywords", "Team", rowNum))
							.equalsIgnoreCase(reader.getCellData("Opp_RRDUser", "TeamName", rowval))) {

						String RRdUser = reader.getCellData("Opp_RRDUser", "SearchPeople", rowval);
						String caseDate = reader.getCellData("Opp_RRDUser", "LastCaseAssignedDate", rowval);
						String Leaddate = reader.getCellData("Opp_RRDUser", "LastLeadAssignedDate", rowval);
						String Oppdate = reader.getCellData("Opp_RRDUser", "LastOppAssignedDate", rowval);
						String Recordlimit = reader.getCellData("Opp_RRDUser", "RecordAssignLimit", rowval);
						String AllowedNo = reader.getCellData("Opp_RRDUser", "Allowedopencases", rowval);
